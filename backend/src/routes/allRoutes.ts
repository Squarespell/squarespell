...
userRouter.get('/plan', async (req: AuthenticatedRequest, res) => {
  const { data: user } = await supabase.from('users').select('plan,quiz_count').eq('id', req.dbUserId).single();
  if (!user) return res.status(404).json({ error: 'Not found' });
  res.json({ plan: user.plan, quiz_count: user.quiz_count, limits: getPlanLimits(user.plan) });
});
