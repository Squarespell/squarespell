...
/**
 * Perform: fetch quiz completions since last poll
 * Called whenever Zapier needs to fetch new completions
 */
export async function perform(z: any, bundle: any): Promise<QuizCompletedPayload[]> {
  try {
    ...
    // Fetch leads (quiz completions) created since last poll
    ...
    // Fetch quiz and outcome information for each completion
    const completions: QuizCompletedPayload[] = [];
    ...
    for (const lead of leads) {
      try {
        ...
        console.error(`Error enriching completion ${lead.id}:`, err);
      }
    }

    return completions;
  } catch (err: any) {
    console.error('quizCompleted trigger error:', err);
    throw err;
  }
}

/**
 * Zapier trigger definition for "Quiz Completed"
 */
export const quizCompleted = {
  ...
  display: {
    ...
    description: 'Triggers when someone completes your quiz.',
    hidden: false,
  },
  operation: {
    type: 'polling',
