...
| `CORS_STRICT_ORIGINS` | Squarespell (server env) | staging: "true" drops the built-in production origins so credentialed CORS allows only CORS_ORIGINS / FRONTEND_URL - unset = built-in defaults (production behaviour unchanged) | backend/src/app.ts | Quiz SaaS | yes | no | Not applicable (unset in production) |
...
| `CONNECT_ENABLED` | Squarespell (server env) | one-button connect feature flag; exactly `true` enables; unset = off (staging review only) | backend/src/services/connect/service.ts | Quiz SaaS | yes | no | Not applicable (unset in production) |
| `CONNECT_MAX_SITES` | Squarespell (server env) | optional limit on connected websites per account; unset = no limit (limits not yet approved) | backend/src/services/connect/service.ts | Quiz SaaS | yes | no | Not applicable (unset) |
| `CONNECT_MAX_INSTALLATIONS_PER_SITE` | Squarespell (server env) | optional limit on quiz installations per website; unset = no limit (limits not yet approved) | backend/src/services/connect/service.ts | Quiz SaaS | yes | no | Not applicable (unset) |
| `CONNECT_TEST_FAULTS` | Squarespell (server env) | staging-fixture only: simulate a publish failure to prove rollback; never set in production | backend/src/routes/connect.ts | Quiz SaaS | yes | no | Not applicable (staging fixture only) |
| `SMTP_HOST` | Hostinger Email | staging: SMTP host for the first-party passwordless auth email service (Render not yet wired) | infra/hostinger/docker-compose.staging.yml, backend/src/services/email/hostingerSmtpProvider.ts | Quiz SaaS | yes | YES - smtp.hostinger.com | Not applicable (staging server env only, not yet on Render/Vercel) |
| `SMTP_PORT` | Hostinger Email | staging: SMTP port, 465 implicit TLS | infra/hostinger/docker-compose.staging.yml, backend/src/services/email/hostingerSmtpProvider.ts | Quiz SaaS | yes | YES - 465 | Not applicable (staging server env only) |
| `SMTP_SECURE` | Hostinger Email | staging: always true for port 465 | infra/hostinger/docker-compose.staging.yml, backend/src/services/email/hostingerSmtpProvider.ts | Quiz SaaS | yes | YES | Not applicable (staging server env only) |
| `SMTP_USER` | Hostinger Email | staging: mailbox used as the From address for sign-in codes | infra/hostinger/docker-compose.staging.yml, backend/src/services/email/hostingerSmtpProvider.ts, backend/src/services/auth/mailer.ts | Quiz SaaS | yes | YES - info@squarespellquiz.com | Not applicable (staging server env only) |
| `SMTP_PASSWORD` | Hostinger Email | staging: Hostinger app password scoped to this mailbox, never the mailbox login password | infra/hostinger/docker-compose.staging.yml, backend/src/services/email/hostingerSmtpProvider.ts | Quiz SaaS | yes | rotate if ever exposed | Not applicable (staging server env only; stored mode 600 on the VPS, never in a dashboard) |
| `COOKIE_SECURE` | Squarespell (server env) | optional override forcing Secure session/CSRF cookies outside NODE_ENV=production | backend/src/services/auth/sessions.ts | Quiz SaaS | yes | no | Not applicable (unset; defaults to NODE_ENV === production) |
| `EMAIL_TRANSPORT` | Squarespell (server env) | test-only email transport switch; refused at startup when NODE_ENV=production | backend/src/services/email/index.ts | Quiz SaaS | yes | no | Not applicable (never set outside the hermetic test suite) |
