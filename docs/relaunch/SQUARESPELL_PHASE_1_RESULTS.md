...
| `CLERK_EMAIL_FROM` | Resend | optional (staging self-delivery): sender for Clerk verification-code emails relayed through Resend - default "Squarespell Quiz <hello@mail.squarespellquiz.com>"; used only when CLERK_SELF_DELIVERY_ENABLED=true | backend/src/routes/clerkWebhook.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
...
| `CLERK_SELF_DELIVERY_ENABLED` | Squarespell (server env) | feature flag: exactly "true" self-delivers Clerk's verification_code email via Resend instead of Clerk's own sender; unset = off - staging cutover for the shared Clerk dev-sender deliverability issue; keep unset in production until proven | backend/src/routes/clerkWebhook.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
