...
| Deploy config | `render.yaml` (1 web service + 6 crons), `backend/vercel.json` (1 cron), `frontend/vercel.json` (rewrite + embed headers), `.github/workflows/keepalive.yml` | See section 4. |
...
Generated by `node scripts/config-inventory.mjs` (re-runnable; `--check` fails when code references a variable that has no ownership entry, and a permanent test runs it). It scans `backend/src`, the frontend, `render.yaml` and the workflows. **`presence in live dashboards` is `TO_BE_FILLED` for the coordinator** - Phase 1 had no dashboard access. All rows belong to the Quiz SaaS; no marketplace variable is listed. "Purpose" states the environment the variable is meant for; nothing about production values was inspected.
...
|---|---|---|---|---|---|---|---|
| `ADMIN_EMAILS` | Render (app config) | prod: admin dashboard allow-list | backend/.env.example, backend/src/routes/allRoutes.ts, render.yaml | Quiz SaaS | yes | review | TO_BE_FILLED |
| `ANTHROPIC_API_KEY` | Anthropic | prod (Render) | backend/.env.example, backend/.env.staging.example, backend/src/services/aiEmailEngine.ts (+3) | Quiz SaaS | yes | YES - dedicated Quiz key (Phase 0 target) | TO_BE_FILLED |
| `ANTHROPIC_MAX_RETRIES` | Anthropic | optional (Phase 1) retry budget - default 1 | backend/.env.example, backend/src/lib/anthropicClient.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
| `ANTHROPIC_TIMEOUT_MS` | Anthropic | optional (Phase 1) per-call timeout - default 40000 | backend/.env.example, backend/src/lib/anthropicClient.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
| `API_BASE_URL` | Render (app config) | legacy alias for keep-alive self ping | backend/src/index.ts | Quiz SaaS | unclear (alias) | as BACKEND_URL | TO_BE_FILLED |
| `API_URL` | Render (app config) | legacy alias of BACKEND_URL | backend/src/routes/allRoutes.ts, backend/src/routes/unsubscribe.ts, backend/src/services/resultEmail.ts (+1) | Quiz SaaS | unclear (alias) | as BACKEND_URL | TO_BE_FILLED |
| `APP_URL` | Render (app config) | prod: links in emails - default https://app.squarespell.com | backend/.env.example, backend/src/routes/allRoutes.ts, backend/src/services/automationEngine.ts (+3) | Quiz SaaS | yes | YES | TO_BE_FILLED |
| `BACKEND_URL` | Render (app config) | prod: unsubscribe/report links, cron | backend/src/routes/allRoutes.ts, backend/src/routes/unsubscribe.ts, backend/src/services/resultEmail.ts (+3) | Quiz SaaS | yes | YES if API host changes | TO_BE_FILLED |
| `BUSINESS_ADDRESS` | other (CAN-SPAM footer) | prod (Render) - default is a hard-coded Delaware address | backend/src/services/unsubscribe.ts | Quiz SaaS | yes | review | TO_BE_FILLED |
| `CLERK_API_URL` | Clerk | optional (Phase 1): API base override - new in Phase 1; tests use a closed local port | backend/.env.example, backend/src/middleware/auth.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
| `CLERK_JWT_KEY` | Clerk | optional (Phase 1): networkless verification - new in Phase 1; unset = JWKS via Clerk API | backend/.env.example, backend/src/middleware/auth.ts | Quiz SaaS | yes | YES if used (new instance key) | TO_BE_FILLED |
| `CLERK_SECRET_KEY` | Clerk | prod (Render + Vercel) - currently a DEVELOPMENT instance - sk_test/sk_live pair | backend/.env.example, backend/.env.staging.example, backend/src/middleware/auth.ts (+3) | Quiz SaaS | yes | YES - new production Clerk app | TO_BE_FILLED |
| `CLERK_WEBHOOK_SECRET` | Clerk | prod (Render) - webhook may not be registered | backend/.env.example, backend/src/routes/clerkWebhook.ts, render.yaml | Quiz SaaS | yes | YES - new endpoint signing secret | TO_BE_FILLED |
| `CORS_ORIGIN` | Render (app config) | staging example only - code reads CORS_ORIGINS - wrong name in .env.staging.example | backend/.env.staging.example | Quiz SaaS | NO: doc drift | n/a | TO_BE_FILLED |
| `CORS_ORIGINS` | Render (app config) | prod: allowlist - defaults are app./quiz./www. squarespell.com | backend/.env.example, backend/src/app.ts, render.yaml | Quiz SaaS | yes | YES | TO_BE_FILLED |
| `CRON_SECRET` | Render + Vercel + GitHub (shared secret) | prod: authenticates cron calls - unset now fails closed (503) | backend/.env.example, backend/src/index.ts, backend/src/middleware/cronAuth.ts (+3) | Quiz SaaS | yes (3 places must match) | rotate at cutover | TO_BE_FILLED |
| `DISABLE_INPROCESS_EMAIL_QUEUE` | Render (app config) | optional (Phase 1) kill switch for the in-process queue drain | backend/.env.example, backend/src/index.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
| `EMAIL_FROM` | Resend | prod (Render) - default hello@squarespell.com hard-coded fallback | backend/src/routes/allRoutes.ts, backend/src/services/automationEngine.ts, render.yaml | Quiz SaaS | yes | YES - sender on squarespellquiz.com | TO_BE_FILLED |
| `ENCRYPTION_KEY` | Render (app secret) | prod: AES-256-GCM for integration configs | backend/src/utils/encryption.ts, render.yaml | Quiz SaaS | yes | DO NOT rotate without re-encrypting stored configs | TO_BE_FILLED |
| `FRONTEND_URL` | Render (app config) | prod: checkout redirects, CORS | backend/.env.example, backend/.env.staging.example, backend/src/app.ts (+6) | Quiz SaaS | yes | YES - squarespellquiz.com app host | TO_BE_FILLED |
| `LOG_LEVEL` | Render (app config) | prod | backend/src/lib/logger.ts | Quiz SaaS | yes | no | TO_BE_FILLED |
| `MARKETING_URL` | Render (app config) | prod - default https://squarespell.com | backend/.env.example, backend/src/routes/allRoutes.ts, backend/src/services/platformEmails.ts (+1) | Quiz SaaS | yes | YES | TO_BE_FILLED |
| `NEXT_PUBLIC_API_URL` | Vercel | prod/preview: frontend -> API base - default https://squarespell-api.onrender.com hard-coded in ~30 files | frontend/.env.example, frontend/.env.staging.example, frontend/app/admin/page.tsx (+48) | Quiz SaaS | yes | YES (API host) | TO_BE_FILLED |
| `NEXT_PUBLIC_CLERK_AFTER_SIGN_IN_URL` | Clerk | prod (Vercel) | frontend/.env.example | Quiz SaaS | yes | review | TO_BE_FILLED |
| `NEXT_PUBLIC_CLERK_AFTER_SIGN_UP_URL` | Clerk | prod (Vercel) | frontend/.env.example | Quiz SaaS | yes | review | TO_BE_FILLED |
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | Clerk | prod (Vercel) | frontend/.env.example, frontend/.env.staging.example | Quiz SaaS | yes | YES - new production publishable key | TO_BE_FILLED |
| `NEXT_PUBLIC_CLERK_SIGN_IN_URL` | Clerk | prod (Vercel) | frontend/.env.example | Quiz SaaS | yes | review (redirect paths) | TO_BE_FILLED |
| `NEXT_PUBLIC_CLERK_SIGN_UP_URL` | Clerk | prod (Vercel) | frontend/.env.example | Quiz SaaS | yes | review (redirect paths) | TO_BE_FILLED |
| `NEXT_PUBLIC_SENTRY_DSN` | other (Sentry) | prod (Vercel) - optional | frontend/.env.staging.example, frontend/sentry.client.config.ts | Quiz SaaS | unclear | YES | TO_BE_FILLED |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase | prod (Vercel) - same | frontend/.env.example, frontend/.env.staging.example | Quiz SaaS | unclear: declared in frontend/.env.example, no code reference found | no | TO_BE_FILLED |
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase | prod (Vercel) - frontend does not query Supabase directly in the scanned code | frontend/.env.example, frontend/.env.staging.example | Quiz SaaS | unclear: declared in frontend/.env.example, no code reference found | no | TO_BE_FILLED |
| `PEXELS_ACCESS_KEY` | other (Pexels) | prod - optional stock-photo search | backend/src/routes/allRoutes.ts, backend/src/routes/emails.ts, render.yaml | Quiz SaaS | unclear | YES | TO_BE_FILLED |
| `PLATFORM_EMAIL_FROM` | Resend | prod (Render) | backend/src/services/platformEmails.ts, render.yaml | Quiz SaaS | yes | YES | TO_BE_FILLED |
| `RENDER_EXTERNAL_URL` | Render | set in render.yaml (value in file); enables in-process timers | backend/src/index.ts, render.yaml | Quiz SaaS | yes | YES if API host changes | TO_BE_FILLED |
| `REPORT_SECRET` | Render (app secret) | prod: HMAC for PDF report links - default in code if unset - verify | backend/.env.example, backend/src/services/reportToken.ts, render.yaml | Quiz SaaS | yes | rotate at cutover | TO_BE_FILLED |
| `RESEND_API_KEY` | Resend | prod (Render) | backend/.env.example, backend/.env.staging.example, backend/src/routes/allRoutes.ts (+6) | Quiz SaaS | yes (provider/sender domain unverified in Phase 0) | YES - new sender domain/key for squarespellquiz.com | TO_BE_FILLED |
| `RESEND_WEBHOOK_SECRET` | Resend | optional->required (Phase 1): Svix signing secret for /api/webhooks/resend - unset = the Resend webhook endpoint answers 503 | backend/.env.example, backend/src/routes/resendWebhook.ts | Quiz SaaS | yes | YES if the endpoint URL changes | TO_BE_FILLED |
| `SENTRY_DSN` | other (Sentry) | prod (Render) - optional | backend/.env.example, backend/.env.staging.example, backend/src/index.ts (+2) | Quiz SaaS | unclear: Sentry org/project not documented | YES - dedicated Quiz project | TO_BE_FILLED |
| `STRIPE_AGENCY_PRICE_ID` | Stripe | declared only - orphan - code reads STRIPE_BUSINESS_PRICE_ID | backend/.env.example, render.yaml | Quiz SaaS | NO | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_AGENCY_YEARLY_PRICE_ID` | Stripe | declared only - orphan | backend/.env.example, render.yaml | Quiz SaaS | NO | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_BUSINESS_PRICE_ID` | Stripe | prod (Render) | backend/.env.example | Quiz SaaS | NO: render.yaml/.env.example named AGENCY | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_BUSINESS_YEARLY_PRICE_ID` | Stripe | prod (Render) | backend/.env.example | Quiz SaaS | NO (see BUSINESS monthly) | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_CORE_PRICE_ID` | Stripe | prod (Render) - no default; missing = 503 | backend/.env.example | Quiz SaaS | NO: render.yaml/.env.example named STARTER; Stripe catalog is Starter/Pro/Agency | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_CORE_YEARLY_PRICE_ID` | Stripe | prod (Render) | backend/.env.example | Quiz SaaS | NO (see CORE monthly) | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_EMAIL_10000_PRICE_ID` | Stripe | add-on pack | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_EMAIL_1000_PRICE_ID` | Stripe | add-on pack | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_EMAIL_5000_PRICE_ID` | Stripe | add-on pack | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_LEAD_1500_PRICE_ID` | Stripe | add-on pack | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_LEAD_3000_PRICE_ID` | Stripe | add-on pack | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_LEAD_500_PRICE_ID` | Stripe | prod (Render) add-on packs - no matching catalog product seen in Phase 0 | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | unclear | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_PRO_PRICE_ID` | Stripe | prod (Render) | backend/.env.example, render.yaml | Quiz SaaS | partly: name matches, prices differ ($39 Stripe vs $19 app) | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_PRO_YEARLY_PRICE_ID` | Stripe | prod (Render) | backend/.env.example, render.yaml | Quiz SaaS | partly (see PRO monthly) | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_QUIZ_PAYMENT_WEBHOOK_SECRET` | Stripe | optional (Phase 1): quiz-payment endpoint secret - falls back to STRIPE_WEBHOOK_SECRET | backend/.env.example, backend/src/routes/allRoutes.ts | Quiz SaaS | yes | YES if that endpoint is registered | TO_BE_FILLED |
| `STRIPE_SECRET_KEY` | Stripe | prod (Render) - LIVE key of the account shared with the marketplace | backend/.env.example, backend/.env.staging.example, backend/src/routes/allRoutes.ts (+2) | Quiz SaaS | yes (account shared; restrict key scope) | no (same Squarespell Limited account) - consider a restricted key | TO_BE_FILLED |
| `STRIPE_STARTER_PRICE_ID` | Stripe | declared in render.yaml / .env.example only - no code reads it - code reads STRIPE_CORE_PRICE_ID | backend/.env.example, render.yaml | Quiz SaaS | NO: orphan declaration | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_STARTER_YEARLY_PRICE_ID` | Stripe | declared only - orphan | backend/.env.example, render.yaml | Quiz SaaS | NO | Phase 2 decision | TO_BE_FILLED |
| `STRIPE_WEBHOOK_SECRET` | Stripe | prod (Render) - endpoint currently DISABLED - endpoint /api/stripe/webhook | backend/.env.example, backend/src/routes/allRoutes.ts, render.yaml | Quiz SaaS | yes | YES if the endpoint URL changes | TO_BE_FILLED |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase | prod (Render) - server-only secret | backend/.env.example, backend/.env.staging.example, backend/src/db/supabaseClient.ts (+12) | Quiz SaaS | yes | rotate at cutover | TO_BE_FILLED |
| `SUPABASE_URL` | Supabase | prod (Render); dev via local .env - project ref is public; URL only | backend/.env.example, backend/.env.staging.example, backend/src/db/supabaseClient.ts (+12) | Quiz SaaS | yes | no (until DB migration in Phase 3) | TO_BE_FILLED |
| `TURNSTILE_SECRET_KEY` | other (Cloudflare Turnstile) | prod - optional - unset = bot check is a no-op | backend/src/services/turnstile.ts | Quiz SaaS | unclear | YES | TO_BE_FILLED |
| `UNSPLASH_ACCESS_KEY` | other (Unsplash) | prod - optional stock-photo search | backend/src/routes/emails.ts, render.yaml | Quiz SaaS | unclear | YES | TO_BE_FILLED |
| `UPSTASH_REDIS_REST_TOKEN` | other (Upstash Redis) | prod (Render) - optional | backend/.env.example, backend/src/services/rateLimiter.ts, render.yaml | Quiz SaaS | unclear | YES | TO_BE_FILLED |
| `UPSTASH_REDIS_REST_URL` | other (Upstash Redis) | prod (Render) - optional - unset = per-process limiter (Phase 1) | backend/.env.example, backend/src/services/rateLimiter.ts, render.yaml | Quiz SaaS | unclear: account owner not documented | YES if Upstash is kept (dedicated DB) | TO_BE_FILLED |
| `UPSTASH_REDIS_TOKEN` | other (Upstash Redis) | staging example only - code reads UPSTASH_REDIS_REST_TOKEN - wrong name in .env.staging.example | backend/.env.staging.example | Quiz SaaS | NO: doc drift | n/a | TO_BE_FILLED |
| `UPSTASH_REDIS_URL` | other (Upstash Redis) | staging example only - code reads UPSTASH_REDIS_REST_URL - wrong name in .env.staging.example | backend/.env.staging.example | Quiz SaaS | NO: doc drift | n/a | TO_BE_FILLED |
...
|---|---|---|---|---|
| Stripe | `POST /api/stripe/webhook` (`allRoutes.ts`, raw body mounted in `app.ts`) | `STRIPE_WEBHOOK_SECRET` | signature (5-minute tolerance), `503 webhook_not_configured` when the secret is unset, idempotency table `stripe_webhook_events`, unknown/non-Quiz events acknowledged and ignored, unmapped plan = explicit 500 `plan_mapping_missing` | Phase 0: "SQUARESPELL QUIZ" endpoint exists and is **Disabled** - `TO_BE_FILLED` (unchanged) |
| Stripe | `POST /api/webhooks/stripe-quiz-payment` ("pay to see results") | `STRIPE_QUIZ_PAYMENT_WEBHOOK_SECRET`, falls back to `STRIPE_WEBHOOK_SECRET` | signature; separate secret now possible (Stripe issues one per endpoint) | not seen in Phase 0 - `TO_BE_FILLED` |
| Clerk | `POST /api/clerk/webhook` (Svix) | `CLERK_WEBHOOK_SECRET` | Svix signature (tested: valid, replay, wrong secret, missing headers); user upsert is idempotent | `TO_BE_FILLED` |
| Resend | `POST /api/webhooks/resend` (bounces, complaints, opens, clicks) | `RESEND_WEBHOOK_SECRET` (new) | **was unauthenticated**; now Svix signature, `503` when unset | `TO_BE_FILLED` |
...
| Render cron `squarespell-scheduled-sends` | `render.yaml` | `*/5 * * * *` | `POST /api/cron/process-scheduled-sends` | needs `BACKEND_URL`, `CRON_SECRET` |
| Render cron `squarespell-lifecycle-emails` | `render.yaml` | daily 09:00 UTC | `trial-reminders`, `lead-milestones` | as above (script exits 0 even on HTTP errors) |
| Render cron `squarespell-weekly-digest` | `render.yaml` | Mon 10:00 UTC | `weekly-digest` | as above |
| Render cron `squarespell-monthly-report` | `render.yaml` | 1st, 10:00 UTC | `monthly-report` | as above |
| Render cron `squarespell-keep-alive` | `render.yaml` | `*/10 * * * *` | `GET /health` | works |
| In-process timers | `backend/src/index.ts` (only when `RENDER_EXTERNAL_URL`/`API_BASE_URL` is set) | 5 min self-ping; 30 min weekly-digest trigger (**no-op**: path `/cron/weekly-digest` lacks `/api`; the Render cron sends the digest - do not "fix" it or digests double-send); 30 min preview-cache cleanup (path **fixed** in this PR, it was a 404); 4-day Supabase ping; **new: 5-min email-sequence queue drain** | - | see D16; `DISABLE_INPROCESS_EMAIL_QUEUE=true` switches the new drain off |
...
| F11 | The in-process weekly-digest trigger calls a path that does not exist (no-op). Leave it: the Render cron sends the digest. | `index.ts:256` |
...
3. **Cron secret:** `CRON_SECRET` must be identical on Render (API + 5 crons), Vercel and GitHub; unset now fails closed (503) instead of open.
...
10. Review and merge this PR only after the preview deployment is smoke-tested; nothing here deploys itself.
...
4. No preview/staging environment has been smoke-tested (Vercel preview only builds the frontend; the API preview does not exist).
...
| Configuration ownership mapped | **Partly met**: names, owners and gaps mapped; `presence in live dashboards` is `TO_BE_FILLED`. |
...
**Overall: conditional. Do not pilot or migrate until section 11 items 1-4 are done and the smoke test passes against a preview/staging deployment.**
