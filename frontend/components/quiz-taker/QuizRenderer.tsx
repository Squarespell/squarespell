...
/**
 * QuizRenderer — the single, shared presentational renderer for "taking" a
 * quiz: question (all answer layouts), lead gate, submitted, and result
 * stages.
 *
 * This was extracted from app/quiz/[slug]/page.tsx (the live, public quiz
 * page) so that the quiz-funnel builder's Stage 4 preview (TryFlowInner.tsx)
 * can render the EXACT same markup/styles a real visitor sees, instead of
 * maintaining a second, simplified mockup that drifts out of sync.
 *
 * Design constraint: this component is purely presentational. It owns NO
 * data fetching, NO analytics/event tracking, and NO network calls — all of
 * that stays in the call site (page.tsx fetches from the API and tracks
 * events; the builder's Stage 4 simulates everything in memory). Every piece
 * of state (current stage, answers, outcome, etc.) and every side-effecting
 * action (pick an option, go back, submit the lead, etc.) is passed in via
 * props/callbacks.
 *
 * Both app/quiz/[slug]/page.tsx and the quiz-funnel builder's Stage 4 render
 * this component — there is only one quiz-taking UI in the codebase.
 */
...
export type RendererStage = 'question' | 'leadgate' | 'submitted' | 'result';
...
function QuestionMedia({ mediaUrl, mediaType }: { mediaUrl: string; mediaType: string; brandPrimary: string }) {
...
export interface QuizRendererProps {
  ...
  hoverOpt: number | null;
  setHoverOpt: (oi: number | null) => void;
  transitioning: boolean;
  transDir: 'forward' | 'back';

  pickOption: (oi: number) => void;
  goBack: () => void;
  ...
  /* Submitted (email-the-report) stage */
  onViewResultsNow?: () => void;

  /* Result stage extras */
  resultEmail: string;
  setResultEmail: (v: string) => void;
  resultEmailSending: boolean;
  resultEmailSent: boolean;
  resultEmailError: string;
  onSendResultEmail: () => void;
  linkCopied: boolean;
  onCopyLink: () => void;
  ...
  /* Branding/CSS-affecting toggles, all pre-resolved by the caller */
  isProPlan: boolean;
}

export default function QuizRenderer(props: QuizRendererProps) {
  var {
    quiz, slug, stage, isMobile,
    qIdx, answers, outcome, totalScore, timeRemaining, hoverOpt, setHoverOpt, transitioning, transDir,
    pickOption, goBack,
    firstName, setFirstName, email, setEmail, consentGiven, setConsentGiven, submitting, submitLead, leadError,
    onViewResultsNow,
    resultEmail, setResultEmail, resultEmailSending, resultEmailSent, resultEmailError, onSendResultEmail,
    linkCopied, onCopyLink, couponCopied, onCopyCoupon, countdown, onCancelCountdown,
    pdfGenerating, onDownloadPdf, shareUrl,
    isProPlan,
  } = props;
        ...
        {stage === 'question' && currentQ && (
          <div key={'q-' + qIdx} style={{
            animation: transitionType === 'none' ? 'none' :
              ...
              transDir === 'forward' ? 'sq-slide-in-fwd 0.35s ease-out' : 'sq-slide-in-back 0.35s ease-out',
          }}>
            {/* Header */}
          ...
          <div ref={resultCardRef} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{
              ...
              {showSocialSharing && (
                <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 20, flexWrap: 'wrap' }}>
                  {[
                    { name: 'Twitter', icon: '𝕏', getUrl: function() { return 'https://twitter.com/intent/tweet?text=' + encodeURIComponent('I got "' + outcome.title + '"! Take the quiz: ' + shareUrl); } },
                    { name: 'Facebook', icon: 'f', getUrl: function() { return 'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(shareUrl); } },
                    { name: 'LinkedIn', icon: 'in', getUrl: function() { return 'https://www.linkedin.com/sharing/share-offsite/?url=' + encodeURIComponent(shareUrl); } },
                  ].map(function(s) {
                    return (
                  ...
                  <button type="button" onClick={onCopyLink} style={{
            ...
            {showCoupon && (outcome.couponCode || outcome.coupon_code) && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  ...
                  <button type="button" onClick={onCopyCoupon} style={{
            ...
            {showCountdownTimer && redirectUrl && countdown > 0 && (
              <div style={{
                ...
                Redirecting in {countdown}s... <button type="button" onClick={onCancelCountdown}
            ...
            {showPdfDownload && (
              <div style={{ textAlign: 'center', paddingTop: 4 }}>
                <button type="button" disabled={pdfGenerating} onClick={onDownloadPdf} data-pdf-trigger="true" style={{
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
