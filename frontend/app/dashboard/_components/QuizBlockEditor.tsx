  ...
  if (block.type === 'question') {
    ...
    return (
      <div>
        ...
        {/* Section 2b: Scoring — per-answer point values */}
        <SidebarSection title="Scoring" defaultOpen={true}
          icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={C.ACCENT} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M12 20V10M18 20V4M6 20v-4" /></svg>}
        >
          <div style={{ fontSize: 12, color: C.TEXT_MUTED, marginBottom: 12, lineHeight: 1.5 }}>
            Assign point values to each answer option. Used for scored outcomes.
          </div>
          {qb.options.map(function(opt, oi) {
            return (
              <div key={opt.id} style={{
                ...
                }}>{opt.text || 'Option ' + String.fromCharCode(65 + oi)}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: C.TEXT_MUTED }}>pts</span>
                  <input
                    ...
                    style={{
                      width: 56, fontSize: 14, fontWeight: 700, color: C.ACCENT,
                      fontFamily: C.FONT + ',system-ui,sans-serif',
          ...
          <div style={{
            marginTop: 10, padding: '10px 12px', background: C.ACCENT_LIGHT, borderRadius: 10,
            ...
            <span style={{ fontSize: 11, color: C.ACCENT, fontWeight: 600 }}>
              Total: {qb.options.reduce(function(sum, o) { return sum + (o.score || 0); }, 0)} pts across {qb.options.length} options
            </span>
          </div>
        </SidebarSection>
        ...
        {/* Section 5: Timer — always visible */}
        <SidebarSection title="Timer" defaultOpen={true}
          icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={C.ACCENT} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx={12} cy={12} r={10} /><polyline points="12 6 12 12 16 14" /></svg>}
        >
          <div style={{ fontSize: 12, color: C.TEXT_MUTED, marginBottom: 10, lineHeight: 1.5 }}>
            Set a countdown timer for this question. 0 = no limit.
          </div>
          <div style={{
            ...
            <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={qb.timeLimit ? C.ACCENT : C.TEXT_MUTED} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
            ...
            <input
              ...
              style={{
                width: 80, fontSize: 16, fontWeight: 700, color: qb.timeLimit ? C.ACCENT : C.TEXT_MUTED,
                fontFamily: C.FONT + ',system-ui,sans-serif',
                border: '1.5px solid ' + C.BORDER, borderRadius: 8,
                padding: '8px 8px', background: '#fff', outline: 'none',
                textAlign: 'center',
              }}
              min={0}
              onFocus={function(e) { e.currentTarget.style.borderColor = C.ACCENT; }}
              onBlur={function(e) { e.currentTarget.style.borderColor = C.BORDER; }}
            />
            <span style={{ fontSize: 14, fontWeight: 600, color: C.TEXT_MUTED }}>seconds</span>
          </div>
          {qb.timeLimit > 0 && (
            <div style={{
              marginTop: 8, padding: '8px 12px', background: C.ACCENT_LIGHT, borderRadius: 8,
              ...
              Timer active: {qb.timeLimit}s per question
            </div>
          )}
        </SidebarSection>

        {/* Section 6: Answer Explanations — collapsed */}
        <SidebarSection title="Answer Explanations" defaultOpen={false}
          icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={C.TEXT_MUTED} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx={12} cy={12} r={10} /><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" /><line x1={12} y1={17} x2={12.01} y2={17} /></svg>}
        >
          <div style={{ fontSize: 12, color: C.TEXT_MUTED, marginBottom: 10, lineHeight: 1.5 }}>
            Show an explanation after the user picks each answer.
          </div>
          {qb.options.map(function(opt, oi) {
            return (
              <div key={opt.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C.ACCENT, width: 20, textAlign: 'center' }}>{String.fromCharCode(65 + oi)}</span>
                <input
