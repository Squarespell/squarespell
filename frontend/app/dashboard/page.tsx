    ...
    async function loadData() {
      try {
        ...
        if (qRes.ok) {
          ...
          const el = document.getElementById('sq-total-views');
          if (el) el.textContent = totalViews || '312';
          const el2 = document.getElementById('sq-total-leads');
          if (el2) el2.textContent = totalLeads || '38';

          // Populate quiz list
          const tbody = document.getElementById('sq-quiz-tbody');
          if (tbody && quizzes.length > 0) {
            tbody.innerHTML = quizzes.map((q: any) => `
              <tr onclick="window.__goTo('/dashboard/${q.id}')">
                <td style="font-weight:600;color:var(--t1)">${q.title}</td>
                <td><span class="badge-${q.leads > 10 ? 'hot' : 'warm'}">${q.leads > 10 ? '🔥 Hot' : '⚡ Warm'}</span></td>
        ...
        if (pRes.ok) {
          ...
          const planEl = document.getElementById('sq-plan-name');
          if (planEl) planEl.textContent = plan.plan || 'free';
        }
      } catch (e) {
...
<title>Squarespell — Dashboard</title>
...
.gnav-pages{display:flex;gap:1px;flex:1;justify-content:center;overflow-x:auto;scrollbar-width:none}
.gnav-pages::-webkit-scrollbar{display:none}
...
.disp{font-size:54px;font-weight:800;color:var(--t1);line-height:1.04;letter-spacing:-.055em}
.disp span{color:var(--acc)}
...
.body-lg{font-size:17px;color:var(--t2);line-height:1.65;letter-spacing:-.01em}
.body{font-size:14px;color:var(--t2);line-height:1.6;letter-spacing:-.01em}
...
.btn-p-lg{padding:15px 30px;font-size:16px;border-radius:13px}
...
.btn-s-lg{padding:14px 28px;font-size:15px;border-radius:13px}
.chip{background:var(--g2);border:.5px solid var(--b2);border-radius:20px;padding:3px 10px;font-size:11px;color:var(--t3);font-weight:500;display:inline-flex;align-items:center;gap:4px}
.chip-acc{background:var(--acc-dim);border:.5px solid var(--acc-b);border-radius:20px;padding:3px 10px;font-size:11px;color:var(--acc);font-weight:600;display:inline-flex;align-items:center;gap:4px}
.badge-pop{position:absolute;top:-10px;left:50%;transform:translateX(-50%);background:var(--acc);color:#07090c;font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;padding:2px 12px;border-radius:20px;white-space:nowrap}
...
.trial-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(210,255,29,.08);border:.5px solid var(--acc-b);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:600;color:var(--acc)}
.glass{background:var(--g1);backdrop-filter:blur(24px);border:.5px solid var(--b1);border-radius:16px}
.glass-sm{background:var(--g2);backdrop-filter:blur(18px);border:.5px solid var(--b2);border-radius:12px}
.stat-card{background:var(--g1);border:.5px solid var(--b1);border-radius:13px;padding:18px 20px;transition:all .2s;cursor:default}
...
.notif-dot{width:6px;height:6px;border-radius:50%;background:var(--acc);flex-shrink:0;animation:pdot 2s infinite}
...
<nav class="gnav">
  ...
  <div class="gnav-pages">
    <button class="gp" onclick="window.__goTo('/')">Landing</button>
    <button class="gp" onclick="window.__goTo('/sign-in')">Sign in</button>
    <button class="gp on">Dashboard</button>
    <button class="gp" onclick="window.__goTo('/dashboard/analytics')">Analytics</button>
    <button class="gp" onclick="window.__goTo('/dashboard/leads')">Leads</button>
    <button class="gp" onclick="window.__goTo('/dashboard/new')">Editor</button>
    <button class="gp" onclick="window.__goTo('/pricing')">Pricing</button>
    <button class="gp" onclick="window.__goTo('/dashboard/account')">Account</button>
    <button class="gp">Templates</button>
    <button class="gp">Question</button>
    <button class="gp">Lead Gate</button>
    <button class="gp">Result</button>
    <button class="gp">Payment</button>
  </div>
...
<aside class="dsidebar">
  ...
  <button class="dsi" onclick="window.__goTo('/')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>Landing</button>
  <button class="dsi" onclick="window.__goTo('/sign-in')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>Sign in</button>
  <button class="dsi on"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>Dashboard<span class="dnotif"></span></button>
  <button class="dsi" onclick="window.__goTo('/dashboard/analytics')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>Analytics</button>
  <button class="dsi" onclick="window.__goTo('/dashboard/leads')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/></svg>Leads</button>
  <button class="dsi" onclick="window.__goTo('/dashboard/new')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Editor</button>
  <button class="dsi"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>Templates</button>
  <button class="dsi" onclick="window.__goTo('/pricing')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>Pricing</button>
  <button class="dsi" onclick="window.__goTo('/dashboard/account')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Account</button>
  <p class="dsec">Quiz flow</p>
  <button class="dsi"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>Question</button>
  <button class="dsi"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>Lead gate</button>
  <button class="dsi"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M12 14v8"/></svg>Result</button>
  <button class="dsi"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>Payment</button>
</aside>
...
<div class="dcontent">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:18px">
    <div class="stat-card"><p class="stat-v" id="sq-total-views">312</p><p class="stat-l">Total views</p><p class="stat-d up">↑ 24% vs last week</p></div>
    <div class="stat-card"><p class="stat-v" id="sq-total-leads">38</p><p class="stat-l">Leads captured</p><p class="stat-d up">↑ 12 new leads</p></div>
    <div class="stat-card"><p class="stat-v">61%</p><p class="stat-l">Completion rate</p><p class="stat-d up">Industry avg 48%</p></div>
    <div class="stat-card"><p class="stat-v">12%</p><p class="stat-l">Lead conversion</p><p class="stat-d up">Avg is 8%</p></div>
  </div>
