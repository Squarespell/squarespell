...
export default function LeadsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(function() { console.error('[Squarespell Quiz] Leads error', error); }, [error]);
    ...
    <DashboardShell title="Error">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 200px)', padding: 40 }}>
        <div style={{ maxWidth: 420, textAlign: 'center' }}>
          ...
          <h2 style={{ margin: '0 0 10px', fontSize: 20, fontWeight: 700, color: C.TEXT }}>Leads Error</h2>
          <p style={{ margin: '0 0 20px', fontSize: 14, color: C.TEXT_MUTED }}>Something went wrong loading leads. Your data is safe.</p>
