...
export function TryFlowInner({
  ...
  var handleCreateFromTemplate = useCallback(async function(templateId: string) {
    ...
    try {
      ...
      try {
        ...
        // Not logged in — open full editor, gate only on Publish
      }

      setQuiz(normalizedQuiz);
      setSelectedIdx(0);
      setEditorBlocks(legacyToBlocks({ questions: normalizedQuiz.questions, outcomes: normalizedQuiz.outcomes, leadGate: normalizedQuiz.leadGate }));
      setStage(3);
  ...
  const buildQuiz = useCallback(async () => {
    ...
    try {
      ...
      // Anonymous user: open full QuizBlockEditor (no auth needed to edit; auth gates Publish)
      setQuiz(normalizedQuiz);
  ...
  return (
    <div className="flow-root">
      ...
      <div className={`stage${stage === 2 ? ' active' : ''}`} id="stage-2">
        ...
        {/* Loading skeleton — split panel (mirrors the brand substep layout) */}
        {loading && (
          <div className="s2-split-wrap">
        ...
        {/* Choose substep — split panel */}
        {!loading && sessionToken && s2SubStep === 'choose' && (
          <div className="s2-split-wrap">
        ...
        {/* Building substep — split panel */}
        {!loading && sessionToken && s2SubStep === 'building' && (
          <div className="s2-split-wrap">
            ...
            <div className="s2-left-panel">
              ...
              <div className="s2-left-body">
                <div className="s2-left-analyzing" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div className="s2-left-build-pulse" />
                  Building
                </div>
                <div className="s2-left-site-name">{brand?.site_name || domain}</div>
                {brand?.business?.type && (
      ...
      <div className={`stage${stage === 3 ? ' active' : ''}`} id="stage-3">
        ...
        {/* Full QuizBlockEditor — same experience as the dashboard editor.
            For anonymous users, Publish triggers sign-up. For logged-in users
            the editor auto-saves and Publish is handled by the dashboard. */}
        {editorBlocks.length > 0 && (
          <QuizBlockEditor
            blocks={editorBlocks}
            onChange={function(b) { setEditorBlocks(b); }}
            settings={{}}
            userPlan="free"
            backUrl=""
            saveState="idle"
