    ...
    <div style={{ minHeight: '100vh', background: '#07090c', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: '"DM Sans", system-ui, sans-serif', padding: '40px 24px' }}>
      <div style={{ width: '100%', maxWidth: '480px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '48px' }}>
          ...
          <span style={{ fontSize: '28px', fontWeight: 800, color: '#f0f2f5', letterSpacing: '-0.03em' }}>Squarespell</span>
        </div>
        ...
        {step === 'form' ? (
          <>
            ...
            {fromTry && (
              <div style={{ marginBottom: '28px', background: 'rgba(210,255,29,0.08)', border: '1px solid rgba(210,255,29,0.2)', borderRadius: '10px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
              <button onClick={handleGoogleSignUp} disabled={googleLoading} style={{ flex:1, height:'44px', background: googleLoading ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'10px', color:'#f0f2f5', fontSize:'14px', fontWeight:500, fontFamily:'"DM Sans",system-ui,sans-serif', cursor: googleLoading ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', transition:'all 0.15s' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', height: '44px', background: '#D2FF1D', color: '#07090c', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: 700, fontFamily: '"DM Sans", system-ui, sans-serif', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              ...
              <h1 style={{ fontSize: '28px', fontWeight: 800, color: '#f0f2f5', letterSpacing: '-0.04em', margin: '0 0 10px' }}>Check your email</h1>
            ...
            <form onSubmit={handleVerify}>
              <div style={{ marginBottom: '24px' }}>
                ...
                <input type="text" placeholder="Enter 6-digit code" value={code} onChange={e => setCode(e.target.value)} style={{ ...inputStyle, textAlign: 'center' as const, fontSize: '20px', letterSpacing: '0.3em' }} maxLength={6} required />
              </div>
              {error && <p style={{ fontSize: '14px', color: '#ff6b6b', marginBottom: '16px', textAlign: 'center' }}>{error}</p>}
              <button type="submit" disabled={loading} style={{ width: '100%', height: '44px', background: '#D2FF1D', color: '#07090c', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: 700, fontFamily: '"DM Sans", system-ui, sans-serif', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
