#!/usr/bin/env bash
# Apply the FULL migration chain to the STAGING database (quiz_staging) on the private db container.
# Order matches the Phase 1 CI proof: SUPABASE_SCHEMA.sql, backend/migrations/*.sql (002..031, incl. the
# forward-only 031 reconciliation), then backend/src/db/migrations/20260415_email_automation.sql.
# Re-runnable: applied files are recorded in ops.applied_migrations and skipped. It only talks to the local
# staging db container - never to Supabase or any production database.
set -euo pipefail
...
q() { "${DC[@]}" exec -T db psql -q -v ON_ERROR_STOP=1 -U postgres -d quiz_staging "$@"; }

echo "== roles, auth stubs, bookkeeping =="
"${DC[@]}" exec -T -e AUTHENTICATOR_PASSWORD db sh -c 'psql -q -v ON_ERROR_STOP=1 -U postgres -d quiz_staging -v pw="$AUTHENTICATOR_PASSWORD"' <<'SQL'
